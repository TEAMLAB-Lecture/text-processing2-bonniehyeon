...
def digits_to_words(input_string):
    ...
    * 만약 인풋 스트링에서 숫자가 존재하지 않는 다면, 빈 문자열 (empty string)을 반환함
    ...
    """
    numlist = list(filter(str.isdigit, input_string))
    num2eng = {'1': 'one', '2': 'two', '3': 'three', '4': 'four', '5': 'five', '6': 'six', 
'7': 'seven', '8': 'eight', '9': 'nine', '0' : 'zero'}
    engnumlist = [num2eng[i] for i in numlist]
    digit_string = ' '.join(engnumlist)
    return digit_string
...
def to_camel_case(underscore_str):
    ...
    * 만약 어떤 변수 이름이 underscore로만 이루어 진다면, 빈 문자열만 반환해도 됨
    ...
    stringProcess = underscore_str.split('_')
    
    if len(stringProcess) <= 1:
         
        return underscore_str         
    stringProcess = ' '.join(stringProcess).split()
    
    if len(stringProcess) == 1:
        
        return stringProcess[0] 
    elif len(stringProcess) == 0: return '' 
        
    camelcase_str = [stringProcess[0].lower()]
    camelcase_str += [i.capitalize() for i in stringProcess[1:]]
